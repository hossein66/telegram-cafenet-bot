...
def get_categories():
    with get_db() as conn:
        cur = conn.cursor()
        cur.execute(
            "SELECT id, name, icon, sort, is_enable as isEnable FROM categories ORDER BY sort"
        )
        rows = cur.fetchall()
...
def get_services(
    ...
    with get_db() as conn:
        ...
        query += f" LIMIT {limit + 1}"  # Fetch one extra to check if more exist
        ...
        if has_more and items:
            last = items[-1]
            # Use sort value and id for cursor
            sort_val = last.get("sort", 0)
            next_cursor = f"{sort_val}|{last['serviceId']}"
